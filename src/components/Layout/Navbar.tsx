...
function Navbar({ setSearchOpen }: NavbarProps) {
  ...
  const avatarColor = user?.isGuest ? "bg-gray-600" : "bg-emerald-500";
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                ...
                {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-emerald-500 rounded-full" />}
              </button>
            );
          })}
        </nav>
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 z-40 bg-gray-950 border-b border-gray-800">
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <div className="flex items-center gap-2">
            ...
            {user && (
              <div className={`w-7 h-7 ${avatarColor} rounded-full flex items-center justify-center text-white text-xs font-bold`} aria-hidden="true">
                {userInitial}
              </div>
            )}
            <button
              ...
              <nav className="flex-1 px-3 py-4 flex flex-col gap-1">
                {navLinks.map((link, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className={isActive ? "text-emerald-400" : "text-gray-500"}>{link.icon}</span>
                      {link.label}
                    </motion.button>
                  );
                })}
