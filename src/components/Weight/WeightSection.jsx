  ...
  return (
    <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-bold text-gray-200">⚖️ {t("weightTitle")}</h3>
          {latest && (
            <p className="text-sm text-gray-500">
              {t("weightLast")} <span className="text-emerald-400 font-semibold">{latest.weight} kg</span> — {formatDate(latest.date)}
            </p>
          )}
        </div>
      ...
      {weights.length === 0 ? (
        ...
        <>
          ...
          <div className="mt-4 flex flex-col gap-2">
            {sorted.map((w) => (
              <div key={w.id} className="flex items-center justify-between text-sm bg-gray-800 rounded-xl px-3 py-2">
                <span className="text-gray-400">{formatDate(w.date)}</span>
                <span className="font-bold text-emerald-400">{w.weight} kg</span>
                {w.notes && <span className="text-gray-500 text-xs">{w.notes}</span>}
